  ...
  try {
    ...
    for (const thread of threadList) {
      ...
      try {
        ...
        logger.error(`Error fetching thread info for ${thread.threadID}: ${err.message}`);
      }
    }
      ...
      await new Promise((resolve, reject) => {
        serverInstance.close((err) => {
          if (err) {
            ...
            reject(new Error(`Failed to close Express server: ${err.message}`));
          } else {
...
async function startBot() {
  try {
    ...
    app.post('/api/login', (req, res) => {
      ...
      const configUsername = config.bot?.login?.username;
      const configPassword = config.bot?.login?.password;

      if (!config.bot) {
        logger.error('Bot section not found in config.json');
        return res.status(500).json({ success: false, message: 'Server configuration error: Missing "bot" section in config.json' });
      }
      if (!config.bot.login) {
        logger.error('Login section not found in config.json under bot');
        return res.status(500).json({ success: false, message: 'Server configuration error: Missing "login" section in config.json under "bot"' });
      }
      if (!configUsername) {
        logger.error('Username not found in config.json bot.login section');
        return res.status(500).json({ success: false, message: 'Server configuration error: Missing "username" in config.json bot.login section' });
      }
      if (!configPassword) {
        logger.error('Password not found in config.json bot.login section');
        return res.status(500).json({ success: false, message: 'Server configuration error: Missing "password" in config.json bot.login section' });
      }
    ...
    app.get('/api/threads', isAuthenticated, async (req, res) => {
      try {
        ...
        const threadList = await new Promise((resolve, reject) => {
          apiInstance.getThreadList(100, null, ["INBOX"], (err, list) => {
            if (err) reject(new Error(`Thread list fetch failed: ${err.message}`));
            else resolve(list);
          });
        });

        const totalItems = threadList.length;
        const totalPages = Math.ceil(totalItems / limit);

        const paginatedThreads = threadList.slice(startIndex, startIndex + limit);

        const threadsWithInfo = await Promise.all(paginatedThreads.map(async (thread) => {
          const info = await new Promise((resolve, reject) => {
            apiInstance.getThreadInfo(thread.threadID, (err, info) => {
              if (err) reject(new Error(`Thread info fetch failed for ${thread.threadID}: ${err.message}`));
              else resolve(info);
            });
          });
    ...
    app.get('/api/users', isAuthenticated, async (req, res) => {
      try {
        ...
        const totalItems = userDocs.length;
        const totalPages = Math.ceil(totalItems / limit);

        const paginatedUserDocs = userDocs.slice(startIndex, startIndex + limit);

        const userIds = paginatedUserDocs.map(doc => doc.userId);
        const userInfo = await new Promise((resolve, reject) => {
          if (!apiInstance) {
            reject(new Error('API instance not initialized'));
            return;
          }
          apiInstance.getUserInfo(userIds, (err, info) => {
            if (err) reject(new Error(`User info fetch failed: ${err.message}`));
            else resolve(info);
          });
        });

        const usersWithInfo = paginatedUserDocs.map(doc => {
          const userId = doc.userId;
          return {
            userId,
            name: userInfo[userId]?.name || "Unknown User",
    ...
    app.post('/api/update-balance', isAuthenticated, async (req, res) => {
      const { userId, balance } = req.body;
      try {
        if (!userId || balance === undefined) {
          throw new Error('User ID and balance are required');
        }
        const usersCollection = db.collection('users');
        await usersCollection.updateOne(
          { userId },
        ...
        res.status(500).json({ success: false, message: `Failed to update balance: ${error.message}` });
      }
    });

    app.post('/api/toggle-ban', isAuthenticated, async (req, res) => {
      const { userId, ban } = req.body;
      try {
        if (!userId || ban === undefined) {
          throw new Error('User ID and ban status are required');
        }
        const usersCollection = db.collection('users');
        await usersCollection.updateOne(
          { userId },
        ...
        res.status(500).json({ success: false, message: `Failed to toggle ban: ${error.message}` });
      }
    });
    ...
    app.post('/api/update-config', isAuthenticated, (req, res) => {
      try {
        const newConfig = req.body;
        fs.writeFileSync(path.join(__dirname, 'config/config.json'), JSON.stringify(newConfig, null, 2));
    ...
    app.post('/api/restart', isAuthenticated, async (req, res) => {
      try {
        logger.info('Restart request received');
        await stopBot();
        logger.info('Bot stopped, now starting again...');
        await startBot();
        logger.info('Bot restart completed');
        res.json({ success: true, message: 'Bot restarted successfully' });
      } catch (error) {
        logger.error(`Error restarting bot: ${error.message}`);
    ...
    app.post('/api/send-message', isAuthenticated, async (req, res) => {
      ...
      if (!threadID || !message) {
        logger.warn('Send message request missing threadID or message');
        return res.status(400).json({ success: false, message: 'Thread ID and message are required' });
      }
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          apiInstance.sendMessage(message, threadID, (err) => {
            if (err) {
              logger.error(`Failed to send message to thread ${threadID}: ${err.message}`);
              reject(new Error(`Send message failed: ${err.message}`));
            } else {
              logger.info(`Message sent to thread ${threadID}: ${message}`);
              resolve();
            }
          });
        });
